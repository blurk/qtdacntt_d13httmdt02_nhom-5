...
export default function ProductScreen({ match, history }) {
  ...
  const { success: successProductReivew, error: errorProductReview } = useSelector(
    (state) => state.productCreateReivew
  );
        ...
        <>
              ...
              <Card>
                <ListGroup variant="flush">
                  ...
                  <ListGroup.Item>
                    <Row>
                      <Col>{t('status')}: </Col>
                      <Col>{product.countInStock > 0 ? t('stock.in') : t('stock.out')}</Col>
                    </Row>
                  </ListGroup.Item>
                  {/* QUANTITY SELECET */}
          ...
          <Row>
            <Col md={6}>
              ...
              {product.reviews.length === 0 && <Message>{t('review.no')}</Message>}
              <ListGroup variant="flush">
                ...
                <ListGroup.Item>
                  ...
                  {errorProductReview && (
                    <Message variant="danger">{t('productAlreadyReview')}</Message>
                  )}
                  {userInfo ? (
                    ...
                    <Message>
                      {t('review.message.p1')} <Link to="/login"> {t('button.signIn')}</Link>{' '}
                      {t('review.message.p2')}
                    </Message>
                  )}
                </ListGroup.Item>
